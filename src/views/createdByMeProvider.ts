...
export class CreatedByMeProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
    ...
    public setHasChanges(value: boolean): void {
        ...
        // 更新上下文，使同步按钮显示/隐藏
        vscode.commands.executeCommand('setContext', 'code-river.createdByMe.hasChanges', value);
    }
    ...
    async getChildren(element?: vscode.TreeItem): Promise<vscode.TreeItem[]> {
        if (element) {
            ...
            return this.getCreatedByMeIssues();
        }
    }

    private async getCreatedByMeIssues(): Promise<IssueTreeItem[]> {
        ...
        // 获取当前用户创建的问题
        const myIssues = issues.filter(issue => issue.author === userName);
